...
const authClient = new PingOneClient({
  ...
  region: 'NorthAmerica',
  redirectUri: `${process.env.API_URL || 'http://localhost:3002'}/auth/callback`,
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', app: 'app-b', timestamp: new Date().toISOString() });
});
...
app.get('/api/protected', (req, res) => {
  ...
  res.json({
    message: `Welcome to App B, ${req.session.user.given_name || req.session.user.email}!`,
    user: req.session.user,
